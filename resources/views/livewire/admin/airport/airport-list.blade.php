<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h4 mb-0">AirPort List</h1>

        <div class="d-flex gap-2">
            <a href="{{ route('admin.airport.create') }}" class="btn btn-dark">
                + Create AirPort
            </a>
            ...
            <tbody>
                @forelse($airports as $airport)
                    <tr>
                        ...
                        <td>{{ $airport->code }}</td>
                        <td>
                            <span class="{{ $airport->status->color() }}">
                                {{ $airport->status->label() }}
                            </span>
                        </td>
                        <td>{{ $airport->created_at->format('m/d/Y h:i a') }}</td>
                        <td class="position-relative">
                            <div class="dropdown">
                                <button class="btn btn-light btn-sm dropdown-toggle" type="button" id="dropdownMenuButton{{ $airport->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                    ⋮
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $airport->id }}">
                                    <li>
                                        <a class="dropdown-item" href="{{ route('admin.airport.edit', $airport->id) }}">Edit</a>
                                    </li>
                                </ul>
                            </div>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted">No airports found</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
